...
## Overview
The `translations` resource provides meaning translations of the Quran in over 40 languages. Like the Quran text, these are segmented by Page (1-604) to allow perfect synchronized scrolling with the Mushaf.
...
```toon
quran[{size}]{c,v,t}:
  {surah_id},{ayah_num},{translated_text}
```

**Index File (`translations.toon`) Schema:**
```toon
translations[{size}]{no,id,name,author,lang,dir,path}:

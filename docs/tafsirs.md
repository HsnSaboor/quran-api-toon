...
```toon
tafsir[{size}]{c,v,t}:
  {surah_id},{ayah_num},{tafsir_text}
```

**Index File (`tafsirs.toon`) Schema:**
```toon
tafsirs[{size}]{no,id,name,author,lang,path}:
  {serial},{slug},{name},{author},{lang},{path}
```

*Note: Tafsir text can be very long (sometimes multiple paragraphs).*
